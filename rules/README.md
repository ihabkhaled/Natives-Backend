...
These engineering rules sit **inside** the governance lifecycle defined by the root [`/claude.md`](../claude.md) (the SDLC operating brain). The SDLC tells you *which phases and artifacts* a change must pass through; these rules tell you *how the code itself must be written*.
...
4. Make the minimal safe change in the correct layer.
5. Run lint → typecheck → test → coverage → build until green.
6. Update docs and the relevant SDLC artifacts ([`/docs`](../docs)).
7. On a new recurring mistake, add it to [`/memory/known-pitfalls.md`](../memory/known-pitfalls.md) and mirror the compact entry across the entry files (`claude.md`, `AGENTS.md`, `codex.md`, `.cursorrules`, `.cursor/rules`).
